...
int main(int argc, char **argv) {
    ...
    int img_size = img1.rows * img1.cols;
    float* color_1 = new float[img_size];
    ...
    for (int i = 0; i < kp1.size(); i++) {
        fh_last->keypoints.push_back(Eigen::Vector2f(kp1[i].pt.x, kp1[i].pt.y));
    }


    Mat img2 = imread(file_2, 0);
    float* color_2 = new float[img_size];
    ...
    fh_cur->makeImages(color_2);


    OpticalFlowTrackerDSO opt_flow_tracker;

    opt_flow_tracker.track(fh_last);
    opt_flow_tracker.track(fh_cur);

    vector<Point2f> origin_pt;
    for (auto &kp: kp1) origin_pt.push_back(kp.pt);
    Mat img2_CV2;
    ...
    for (int i = 0; i < fh_cur->keypoints.size(); i++) {
        ...
        cv::circle(img2_CV2, pt2.pt, 1, cv::Scalar(0, 250, 0), 2);
        cv::line(img2_CV2, origin_pt[i], pt2.pt, cv::Scalar(0, 250, 0));
    }

    cv::imshow("tracked by DSO", img2_CV2);
    ...
    Mat img1_ori;
    cv::cvtColor(img1, img1_ori, cv::COLOR_GRAY2BGR);
    for (int i = 0; i < kp1.size(); i++) {
        if (success_multi[i]) {
            cv::circle(img1_ori, kp1[i].pt, 1, cv::Scalar(250, 0, 0), 2);
        }
    }
    ...
    for (int i = 0; i < kp2_multi.size(); i++) {
        if (success_multi[i]) {
            cv::circle(img2_multi, kp2_multi[i].pt, 1, cv::Scalar(0, 250, 0), 2);
    ...
    for (int i = 0; i < pt2.size(); i++) {
        if (status[i]) {
            cv::circle(img2_CV, pt2[i], 1, cv::Scalar(0, 250, 0), 2);
